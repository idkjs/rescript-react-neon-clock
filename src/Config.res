...
let getReadoutConfig = (~time) => {
  ...
  let nearestMinuteMark = isNearly ? mod(lastMinuteMark + 5, 60) : lastMinuteMark
  let isOClock = nearestMinuteMark === 0
  let isPast = !isOClock && nearestMinuteMark <= 30
  let isTo = !isOClock && !isPast

  let minuteMark = switch (isPast, isOClock) {
  | (true, true) => nearestMinuteMark
  | (_, _) => 60 - nearestMinuteMark
  }
  let nearestHour = isTo || (isOClock && isNearly) ? mod(hours + 1, 24) : hours
  ...
  let isIsOClock = switch (isOClock, !isNoon, !isMidnight) {
  | (true, true, true) => true
  | (_, _, _) => false
  }
...
let timeReadout = (~time) => {
  ...
  let timeText = {
    open Belt
    highlighted->Array.mapWithIndex((idx, h) => h ? Utils.phrases->Array.get(idx) : None)
  }
